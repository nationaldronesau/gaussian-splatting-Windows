...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if iteration % 10 == 0:
            ...
            progress_bar.update(min(10, opt.iterations - iteration))  # Prevent overstepping
...
if __name__ == "__main__":
    ...
    # Start GUI server with timeout to prevent deadlock
    network_gui.init(args.ip, args.port)
    start_time = time.time()
    while network_gui.conn is None and time.time() - start_time < 30:  # 30s timeout
        time.sleep(1)
    if network_gui.conn is None:
        print("Warning: Network GUI connection not established. Proceeding without it.")
